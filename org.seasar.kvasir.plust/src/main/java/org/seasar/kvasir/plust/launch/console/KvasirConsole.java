...
/**
 * Kvasir/Sora plugin console (modified Maven2 plugin Console)
 *
    ...
    implements IPropertyChangeListener, ITraceable
{
        ...
        Maven2Plugin.getStandardDisplay().asyncExec(new Runnable() {
            public void run()
            {
                JFaceResources.getFontRegistry()
                    .addListener(KvasirConsole.this);
                initializeStreams();
                dump();
            }
        });
    ...
    void initializeStreams()
    {
        synchronized (document) {
            if (!initialized) {
                ...
                // install colors
                commandColor = new Color(Maven2Plugin.getStandardDisplay(),
                    new RGB(0, 0, 0));
                commandStream.setColor(commandColor);
                messageColor = new Color(Maven2Plugin.getStandardDisplay(),
                    new RGB(0, 0, 255));
                messageStream.setColor(messageColor);
                errorColor = new Color(Maven2Plugin.getStandardDisplay(),
                    new RGB(255, 0, 0));
    ...
    public void show(boolean showNoMatterWhat)
    {
        if (showNoMatterWhat /*|| showOnMessage*/) {
            if (!visible)
                KvasirConsoleFactory.showConsole();
            else
    ...
    {
        appendLine(ConsoleDocument.MESSAGE, getDateFormat().format(new Date())
            + ": " + message); //$NON-NLS-1$
    }
    ...
    {
        bringConsoleToFront();
        appendLine(ConsoleDocument.ERROR, getDateFormat().format(new Date())
            + ": " + message); //$NON-NLS-1$
    }
